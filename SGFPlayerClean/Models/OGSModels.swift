...
enum SGFParser {
    static func parse(text: String) throws -> SGFTree {
        ...
        func skipWhitespace() { while let c = peek(), c.isWhitespace { advance() } }
        var nodes: [SGFNode] = []
        skipWhitespace()
        ...
        func parseNode() throws -> SGFNode {
            ...
            while let c = peek(), c.isLetter {
                ...
                skipWhitespace()
                while peek() == "[" { values.append(parseValue()); skipWhitespace() }
        ...
        func parseIdent() -> String {
            let start = i; while let c = peek(), c.isLetter { advance() }
        ...
        func parseValue() -> String {
            ...
            while let c = peek() {
                advance()
                if c == "\\" { if let next = peek() { out.append(next); advance() } }
...
struct ChallengeGameInfo: Codable, Hashable {
    let ranked: Bool; let width: Int; let height: Int; let rules: String
}
...
struct OGSChallenge: Identifiable, Decodable {
    ...
    var speedCategory: String {
        ...
        if tpm < 30 { return "blitz" }; if tpm > 3600 * 12 { return "correspondence" }; return "live"
    }
    var timeControlDisplay: String {
        guard let tpm = time_per_move else { return "No limit" }
        if tpm < 60 { return "\(tpm)s / move" }; if tpm < 3600 { return "\(tpm / 60)m / move" }; return "\(tpm / 3600)h / move"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        time_per_move = try? c.decode(Int.self, forKey: .time_per_move)
...
struct SGFCoordinates {
    static func parse(_ s: String) -> (Int, Int)? {
        ...
        let chars = Array(s.lowercased()); let x = Int(chars[0].asciiValue ?? 0) - 97; let y = Int(chars[1].asciiValue ?? 0) - 97
